...
### hstgrism accuracy test (20200911 - present):
- 20200911 -- G102, wavelength tested
- G102, flux tested
- G141

### hstgrism.contamination (20200902 - 20200827):
We developed hstgrism.contamination.Contamination class to handle the computation related contamination. 20200902/01 provides a template for this workflow. In summary, HSTGRISM flow is i) Object run, ii) Contamination run (by using outputs from Object run), and iii) removing contamination. Note that HSTGRISM does not perform the last step, and a user can use Object cps and cps_contaminate in contaminationTrace.csv for this step.

### X01 spectrum (20200903 + 20200826 - 20200821): 
Edited 20200903: X01 G141 was extracted for 1D spectrum. ./20200903/X01_G102_G141_flam.csv is the master file that combined and smoothed X01 1D spectrum.

We investigated X01 in order to get its 1D spectrum. We found that X01 is a standard star existing in various catalogs. However, only photometry from SDSS DR16 ugriz and 2MASS JHK was available. Then, we decided to directly extract it from 201911 and 202001 G102 images simply because X01 was isolated in a flat background; other images (including 201909, 201910, and 202002 from PID 16042) would be more complicated to extract X01. 20200824/X01_201911_G102_rebkg/* and 20200826/X01_202001_G102/* contain its spectra, while 20200826/X01_together_G102/* contains the combined version. The extraction showed consistency across both epochs and photometry.
...
### developing HSTPHOT 1.2.0 (20200820 - 20200818): 
HSTPHOT 1.2.0 was developed and released to both PyPI and Github. Note: An example in 20200821/01 can be used as demo.

### drzblot/crcorr aperture correction: 
This computed aperture drzblot/crcorr aperture correction for grism objects. See 20200808 - 20200731. In summary, drzblot/crcorr aperture correction can be as large as ~2% depending on factors such as i) aperture width, ii) trace alignment, iii) background subtraction, iv) filter, and v) drizzle pattern. Recommendation: halfdy >= 4, and performing drzblot/crcorr computation on an isolated star in the FoV.

## Notes

### 20200911
We extracted Vy2-2 (a planetary nebula for wavelength calibration) to measure accuracy of HSTGRISM to the wavelength mapping in G102. (01) preliminarily explored the dataset, (02) computed the trace/wavelength maps and extracted 1D spectrum (in cps), and (03) identified lines and measured deviations from the standard lines of Vy2-2. The results showed that HSTGRISM maps deviate with median ~14 A (including bad matching which can cause deviation to >100 A). Given G102 resolution ~24 A/pix, this verified that HSTGRISM accurately maps to <0.5 pixel. A deviation at the order of ~1 pixel would be more likely due to bad locating sources in the diret images (i.e., xyd).

Next, we need to measure the wavelength accuracy of G141, and flux accuracy of both G102/G141. Also, increasing the sample size of G102 wavelength accuracy test would be great!

### 20200903
We extracted X01 G141, and combined it with G102 to make a master spectral profile. ./20200903/X01_together_G141/*.csv stacked two epochs of X01 G141. ./20200903/X01_G102_G141_flam.csv combined and smoothed both G102 and G141, and this should be the master file.

### 20200902
hstgrism.contamination.Contamination class was extended its functionality until the end of the flow by providing cps_contaminate in its output file contaminationTrace.csv. This was released with HSTGRISM 8.4.0.

### 20200901
Continued from 20200828, we fixed and demonstrated the flow from contaminateTrace.csv to getting Contaminate cps inside the Object aperture (i.e., cps_contaminate). To do this, we also needed Contaminate spectrum, sensitivity of observing grism, and drzblot apcorr if Object is in drzblot. (Note: drzblot apcorr must have the same halfdy). The flow was straightforward.

Next, we will extend Contamination class with another switch to compute cps_contaminate. The end of 20200828 discussed how to approach this. Also, it laid out the full workflow with HSTGRISM from making an Object run to a Contaminate run.

Note: we also found bugs in GRB190829A/05/* that we forgot to apply drzblot apcorr when computing flam.
